...
protected:
  // BCC_added
  inline uint64_t get_epoch_id(uint64_t tid) const {
    return EpochId(tid);
  }

  inline uint64_t get_num_id_shift() const {
    return NumIdShift;
  }

  inline uint64_t get_core_id(uint64_t tid) const {
    return CoreId(tid);
  }

  inline uint64_t get_core_mask() const {
    return CoreMask;
  }
...
public:
  ...
  transaction_base::tid_t gen_abort_tid() {
    const size_t my_core_id = this->rcu_guard_->guard()->core();
//    COMPILER_MEMORY_FENCE;
//    u_.commit_epoch = ticker::s_instance.global_current_tick();
//    COMPILER_MEMORY_FENCE;
//    return MakeTid(my_core_id, 0, u_.commit_epoch);
    return MakeTid(my_core_id, 0, ticker::s_instance.global_current_tick());
  }
