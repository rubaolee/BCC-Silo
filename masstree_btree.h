...
public:
  ...
  fullscan(const key_type &lower,
            const key_type *upper,
            Callback &callback,std::string*buf =nullptr)const{

        new_low_level_search_range_scanner<false, Callback> scanner(upper, callback);
        threadinfo ti;
        table_.fullscan(lcdf::Str(lower.data(), lower.length()),true, scanner, ti);
    }
 ...
 private:
  ...
  template <bool Reverse, typename Callback> class new_low_level_search_range_scanner;
...
template <typename P>
template <bool Reverse, typename Callback>
class mbtree<P>::new_low_level_search_range_scanner: public search_range_scanner_base<Reverse> {
 public:
  new_low_level_search_range_scanner(const key_type* boundary, Callback & callback)
