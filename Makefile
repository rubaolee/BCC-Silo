...
CXXFLAGS := -g -Wall -std=c++0x #-msse4	# Enable this when using SSE
...
BENCH_SRCFILES = benchmarks/bccbench.cc \
	benchmarks/bcctpcc.cc
...
.PHONY: bcctest
bcctest: $(O)/benchmarks/bcctest

$(O)/benchmarks/bcctest: $(O)/benchmarks/bcctest.o $(OBJFILES) $(MASSTREE_OBJFILES) $(BENCH_OBJFILES) third-party/lz4/liblz4.so
	$(CXX) -o $(O)/benchmarks/bcctest $^ $(BENCH_LDFLAGS) $(LZ4LDFLAGS)
