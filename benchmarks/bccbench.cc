...
bench_runner::run()
{
  ...
  uint64_t *loader_tids = (uint64_t *)malloc(loaders.size() * sizeof(uint64_t));
  memset(loader_tids, 0, sizeof(uint64_t) * loaders.size());
  {
    ...
    const pair<uint64_t, uint64_t> mem_info_before = get_system_memory_info();
    {
      ...
      for (vector<bench_loader *>::const_iterator it = loaders.begin();
          it != loaders.end(); ++it, ++loader_id) {
        (*it)->set_barrier(b);
        (*it)->set_bcc_info(loader_id, loader_tids);
        (*it)->start();
      }
      for (vector<bench_loader *>::const_iterator it = loaders.begin();
          it != loaders.end(); ++it)
  ...
  int launched_threads = loaders.size();
  uint64_t latest_tid = 0;
  for (vector<bench_loader *>::size_type i = 0; i < loaders.size(); ++i) {
      if (latest_tid < loader_tids[i])
        latest_tid = loader_tids[i];
  }
...
//@sh_tid: last committed  tid for each thread.
//@sh_memory: the shared memory among all threads, evenly divided.

#ifdef USE_BCC
  print_bcc_configs();
  uint64_t *tid_arrays_temp[8];
  //int64_t *sh_conflict = new int64_t[nthreads * nthreads];
  //memset(sh_conflict, 0, sizeof(int64_t) * nthreads * nthreads);
  shtxn **sh_txn_ar_temp = (shtxn **)malloc(sizeof(shtxn *) * nthreads);
  memset(sh_txn_ar_temp, 0, sizeof(shtxn *) * nthreads);
#endif
  ...
  for (vector<bench_worker *>::const_iterator it = workers.begin();
       it != workers.end(); ++it/*, ++iworker*/) {
#ifdef USE_BCC
    (*it)->set_bcc_info(launched_threads, nthreads, /*iworker, sh_conflict,*/
        tid_arrays_temp, latest_tid, sh_txn_ar_temp);
#else
    (*it)->set_bcc_info(launched_threads, nthreads, /*iworker, nullptr,*/
        nullptr, 0, nullptr);
#endif
    (*it)->start();
  }
...
//    workers[i]->join();
  ...
  if (verbose) {
    ...
    cerr << "--- txn counter statistics ---" << endl;
    {
      ...
      ndb_wrapper::txn_counter_map agg = workers[0]->get_local_txn_counters();
      for (auto &p : agg) {
        cerr << p.first << ":" << endl;
        for (auto &q : p.second)
