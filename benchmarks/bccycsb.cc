...
public:
      ...
      computation_n(0)
  {
    ...
    uint64_t zipf(uint64_t n, double theta) {
        ...
        double u = (double)(r.next() % 10000000) / 10000000;
        double uz = u * zetan;
        if (uz < 1) return 1;
...
/*
 * This is the default implementation of txn_read()
 * 
...
/*
 * This is the new implementation of txn_read (ycsb-a,ycsb-b,ycsb-c).
 */
  ...
  txn_read()
  {
    ...
    void * const txn = db->new_txn(txn_flags, arena, txn_buf(), HINT_KV_GET_PUT);

#ifdef USE_BCC
    bcc_txn_setup(txn);
#endif
#ifdef BCC_TXN_STATS
  bcc_txn_stats_setup(txn, "bcc_ycsb");
#endif
    ...
    try {
      ...
      for(int i=0;i<ntuples;i++){
        double rp = (r.next()%100)/100.0;
        const uint64_t k = zipf(nkeys-1, gtheta); 
        if(rp<rperc){
        // this is a read
            tbl->get(txn, u64_varkey(k).str(obj_key0), obj_v, -1);
            computation_n += obj_v.size();
...
/*
 * new implementation of txn_write. (ycsb-f: read/read-modified write)
 */
...
/*
  ...
  txn_rmw()
  {
    void * const txn = db->new_txn(txn_flags, arena, txn_buf(), HINT_KV_RMW);
    scoped_str_arena s_arena(arena);
    try {
      const uint64_t key = r.next() % nkeys;
      ALWAYS_ASSERT(tbl->get(txn, u64_varkey(key).str(obj_key0), obj_v,-1));
      computation_n += obj_v.size();
 ...
 */
  ...
  bcc_txn_stats_setup(txn, "bcc_ycsb");
    ...
    try {
      ...
      for(int i=0;i<ntuples;i++){
        double rp = (r.next()%100)/100.0;
        uint64_t k = zipf(nkeys-1, gtheta); 
        if(rp<rperc){
        // this is a read
            tbl->get(txn, u64_varkey(k).str(obj_key0), obj_v, -1);
            computation_n += obj_v.size();
        ...
        // this is an insert 
            tbl->put(txn, u64_varkey(r.next()).str(str()), str().assign(YCSBRecordSize, 'b'));
        }
      }
  ...
  class worker_scan_callback : public ndb_ordered_index::scan_callback {
  public:
  ...
  txn_scan()
  {
    ...
    void * const txn = db->new_txn(txn_flags, arena, txn_buf(), HINT_KV_SCAN);
#ifdef USE_BCC
    bcc_txn_setup(txn);
#endif
#ifdef BCC_TXN_STATS
  bcc_txn_stats_setup(txn, "bcc_ycsb");
    ...
    try {
      ...
      if(rp < rperc){
          tbl->scan(txn, kbegin, &kend, c, s_arena.get());
          computation_n += c.n;
...
#ifdef USE_BCC
      uint64_t latest_tid = ycsb_load_keyrange(
          keystart,
          keyend,
          i,
          db,
...
public:
  ycsb_bench_runner(ndb_wrapper *db, size_t nthreads)
...
private:
...
#ifdef USE_BCC
#ifndef BCC_FIX_YCSB_NUMA
      const int node = numa_node_of_cpu(i);
#else
      const int node = numa_node_of_cpu(::allocator::wid_to_coreid_mapping[i]);
#endif
#else
#ifndef BCC_FIX_YCSB_NUMA
      const int node = numa_node_of_cpu(i);
#else
      const unsigned nsocks = ((nthds + 7) & (~7)) >> 3;
      const int node = numa_node_of_cpu(::allocator::coreids_mapping[i % nsocks][i / nsocks]);
#endif
#endif
  ...
  exclude(const vector<unsigned> &cpus, unsigned nthds)
  {
    ...
    for (auto n : cpus) {
      for (unsigned i = 0; i < nthds; ++i) {
        if (n == ::allocator::wid_to_coreid_mapping[i]) {
          ret.push_back(n);
          break;
        }
      }
    ...
    for (auto n : cpus) {
      for (unsigned i = 0; i < nthds; ++i) {
        if (n == ::allocator::coreids_mapping[i % nsocks][i / nsocks]) {
          ret.push_back(n);
          break;
        }
      }
...
bcctest(ndb_wrapper *db, size_t nthreads,int argc, char **argv)
{
  ...
  while (1) {
    static struct option long_options[] = {
      ...
      {"theta" , required_argument , 0 , 'g'},
      {"ntuples" , required_argument , 0 , 'n'},
      {0, 0, 0, 0}
    };
    int option_index = 0;
    int c = getopt_long(argc, argv, "w:r:g:n:", long_options, &option_index);
    if (c == -1)
      break;
    switch (c) {
    case 0:
  ...
  ycsb_bench_runner r(db,nthreads);
  r.run();
}
