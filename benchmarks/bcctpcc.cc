  ...
  load()
  {
    ...
    try {
      ...
      for (uint i = 1; i <= NumWarehouses(); i++) {
        ...
        n_warehouses++;
        tbl_warehouse(i)->insert(txn, Encode(k), Encode(obj_buf, v)/*,WAREHOUSE*/);
      ...
      for (uint i = 1; i <= NumWarehouses(); i++) {
        const warehouse::key k(i);
        string warehouse_v;
        ALWAYS_ASSERT(tbl_warehouse(i)->get(txn, Encode(k), warehouse_v,-1/*, WAREHOUSE*/));
  ...
  load()
  {
    ...
    try {
      for (uint i = 1; i <= NumItems(); i++) {
        ...
        const size_t sz = Size(v);
        total_sz += sz;
        tbl_item(1)->insert(txn, Encode(k), Encode(obj_buf, v)/*,ITEM*/); // this table is shared, so any partition is OK
      ...
      for (uint b = 0; b < nbatches;) {
        ...
        try {
          ...
          for (uint i = (b * batchsize + 1); i <= iend; i++) {
            ...
            stock_total_sz += sz;
            n_stocks++;
            tbl_stock(w)->insert(txn, Encode(k), Encode(obj_buf, v)/*,STOCK*/);
            tbl_stock_data(w)->insert(txn, Encode(k_data), Encode(obj_buf1, v_data)/*,STOCK_DATA*/);
          }

#ifdef USE_BCC
          if (db->commit_txn(txn, &tid)) {
            if(tid > latest_tid)
                latest_tid = tid;
#else
          if (db->commit_txn(txn,NULL)) {
#endif
            b++;
          } else {
            db->abort_txn(txn);
    ...
    try {
      ...
      for (uint w = 1; w <= NumWarehouses(); w++) {
        ...
        for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++, cnt++) {
          ...
          n_districts++;
          tbl_district(w)->insert(txn, Encode(k), Encode(obj_buf, v)/*,DISTRICT*/);
        ...
        for (uint batch = 0; batch < nbatches;) {
          ...
          try {
            for (uint cidx0 = cstart; cidx0 < cend; cidx0++) {
              ...
              const size_t sz = Size(v);
              total_sz += sz;
              tbl_customer(w)->insert(txn, Encode(k), Encode(obj_buf, v)/*,CUSTOMER*/);
              ...
              tbl_customer_name_idx(w)->insert(txn, Encode(k_idx), Encode(obj_buf, v_idx)/*,CUSTOMER_NAME_IDX*/);
              ...
              tbl_history(w)->insert(txn, Encode(k_hist), Encode(obj_buf, v_hist)/*,HISTORY*/);
            }
#ifdef USE_BCC
            if (db->commit_txn(txn, &tid)) {
              if(tid > latest_tid)
...
tpcc_worker::txn_new_order()
{
  ...
  //   num_txn_contexts : 9
  void *txn = db->new_txn(txn_flags, arena, txn_buf(), HINT_TPCC_NEW_ORDER);
#ifdef USE_BCC
  bcc_txn_setup(txn);
#endif
#ifdef BCC_TXN_STATS
  bcc_txn_stats_setup(txn, "NEW_ORDER");
#endif
  ...
  try {
    ...
    ALWAYS_ASSERT(tbl_customer(warehouse_id)->get(txn, Encode(obj_key0, k_c), obj_v,-1/*, CUSTOMER*/));
    customer::value v_c_temp;
    const customer::value *v_c = Decode(obj_v, v_c_temp);
    checker::SanityCheckCustomer(&k_c, v_c);

    const warehouse::key k_w(warehouse_id);
    ALWAYS_ASSERT(tbl_warehouse(warehouse_id)->get(txn, Encode(obj_key0, k_w), obj_v,-1/*, WAREHOUSE*/));
    warehouse::value v_w_temp;
    const warehouse::value *v_w = Decode(obj_v, v_w_temp);
    checker::SanityCheckWarehouse(&k_w, v_w);

    const district::key k_d(warehouse_id, districtID);
    ALWAYS_ASSERT(tbl_district(warehouse_id)->get(txn, Encode(obj_key0, k_d), obj_v,-1/*, DISTRICT*/));
    ...
    tbl_new_order(warehouse_id)->insert(txn, Encode(str(), k_no), Encode(str(), v_no)/*, NEW_ORDER*/);
    ret += new_order_sz;

    if (!g_new_order_fast_id_gen) {
      district::value v_d_new(*v_d);
      v_d_new.d_next_o_id++;
      tbl_district(warehouse_id)->put(txn, Encode(str(), k_d), Encode(str(), v_d_new)/*, DISTRICT*/);
    }
    ...
    tbl_oorder(warehouse_id)->insert(txn, Encode(str(), k_oo), Encode(str(), v_oo)/*, OORDER*/);
    ret += oorder_sz;

    const oorder_c_id_idx::key k_oo_idx(warehouse_id, districtID, customerID, k_no.no_o_id);
    const oorder_c_id_idx::value v_oo_idx(0);

    tbl_oorder_c_id_idx(warehouse_id)->insert(txn, Encode(str(), k_oo_idx), Encode(str(), v_oo_idx)/*, WAREHOUSE*/);
    ...
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      ALWAYS_ASSERT(tbl_item(1)->get(txn, Encode(obj_key0, k_i), obj_v, -1/*,ITEM*/));
      item::value v_i_temp;
      const item::value *v_i = Decode(obj_v, v_i_temp);
      checker::SanityCheckItem(&k_i, v_i);

      const stock::key k_s(ol_supply_w_id, ol_i_id);
      ALWAYS_ASSERT(tbl_stock(ol_supply_w_id)->get(txn, Encode(obj_key0, k_s), obj_v, -1/*,STOCK*/));
      ...
      tbl_stock(ol_supply_w_id)->put(txn, Encode(str(), k_s), Encode(str(), v_s_new)/*, STOCK*/);
      ...
      tbl_order_line(warehouse_id)->insert(txn, Encode(str(), k_ol), Encode(str(), v_ol)/*, ORDER_LINE*/);
      ret += order_line_sz;
    }
...
tpcc_worker::txn_delivery()
{
  ...
  //   num_txn_contexts : 4
  void *txn = db->new_txn(txn_flags, arena, txn_buf(), HINT_TPCC_DELIVERY);
#ifdef USE_BCC
  bcc_txn_setup(txn);
#endif
#ifdef BCC_TXN_STATS
  bcc_txn_stats_setup(txn, "DELIVERY");
#endif
  ...
  try {
    ...
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      ...
      const new_order::key k_no_1(warehouse_id, d, last_no_o_ids[d-1] + 10);
      new_order_scan_callback new_order_c;
      {
        ANON_REGION("DeliverNewOrderScan:", &delivery_probe0_cg);
        tbl_new_order(warehouse_id)->scan(txn, Encode(obj_key0, k_no_0), &Encode(obj_key1, k_no_1), new_order_c, s_arena.get()/*,NEW_ORDER*/);
      }
      ...
      if (unlikely(!tbl_oorder(warehouse_id)->get(txn, Encode(obj_key0, k_oo), obj_v, -1/*, OORDER*/))) {
      ...
      tbl_order_line(warehouse_id)->scan(txn, Encode(obj_key0, k_oo_0), &Encode(obj_key1, k_oo_1), c, s_arena.get()/*,ORDER_LINE*/);
      float sum = 0.0;
      for (size_t i = 0; i < c.size(); i++) {
        ...
        tbl_order_line(warehouse_id)->put(txn, *c.values[i].first, Encode(str(), v_ol_new)/*, ORDER_LINE*/);
      }
      ...
      tbl_oorder(warehouse_id)->put(txn, Encode(str(), k_oo), Encode(str(), v_oo_new)/*, OORDER*/);
      ...
      tbl_customer(warehouse_id)->put(txn, Encode(str(), k_c), Encode(str(), v_c_new)/*, CUSTOMER*/);
    }
...
tpcc_worker::txn_payment()
{
  ...
  //   num_txn_contexts : 5
  void *txn = db->new_txn(txn_flags, arena, txn_buf(), HINT_TPCC_PAYMENT);
#ifdef USE_BCC
    bcc_txn_setup(txn);
#endif
#ifdef BCC_TXN_STATS
  bcc_txn_stats_setup(txn, "PAYMENT");
#endif
  ...
  try {
    ...
    tbl_warehouse(warehouse_id)->put(txn, Encode(str(), k_w), Encode(str(), v_w_new)/*,WAREHOUSE*/);
    ...
    if (RandomNumber(r, 1, 100) <= 60) {
      ...
      tbl_customer_name_idx(customerWarehouseID)->scan(txn, Encode(obj_key0, k_c_idx_0), &Encode(obj_key1, k_c_idx_1), c, s_arena.get()/*,CUSTOMER_NAME_IDX*/);
      ...
      ALWAYS_ASSERT(tbl_customer(customerWarehouseID)->get(txn, Encode(obj_key0, k_c), obj_v, -1/*,CUSTOMER*/));
      Decode(obj_v, v_c);
    ...
    tbl_customer(customerWarehouseID)->put(txn, Encode(str(), k_c), Encode(str(), v_c_new)/*, CUSTOMER*/);
    ...
    tbl_history(warehouse_id)->insert(txn, Encode(str(), k_h), Encode(str(), v_h)/*, HISTORY*/);
    ret += history_sz;
...
tpcc_worker::txn_order_status()
{
...
#endif
#ifdef BCC_TXN_STATS
  bcc_txn_stats_setup(txn, "ORDER_STATUS");
#endif
  ...
  try {
    ...
    if (RandomNumber(r, 1, 100) <= 60) {
      ...
      static_limit_callback<NMaxCustomerIdxScanElems> c(s_arena.get(), true); // probably a safe bet for now
      tbl_customer_name_idx(warehouse_id)->scan(txn, Encode(obj_key0, k_c_idx_0), &Encode(obj_key1, k_c_idx_1), c, s_arena.get()/*, CUSTOMER_NAME_IDX*/);
      ALWAYS_ASSERT(c.size() > 0);
      INVARIANT(c.size() < NMaxCustomerIdxScanElems); // we should detect this
      int index = c.size() / 2;
      if (c.size() % 2 == 0)
    ...
    if (g_order_status_scan_hack) {
      ...
      const oorder_c_id_idx::key k_oo_idx_1(warehouse_id, districtID, k_c.c_id, numeric_limits<int32_t>::max());
      {
        ...
        tbl_oorder_c_id_idx(warehouse_id)->scan(txn, Encode(obj_key0, k_oo_idx_0), &Encode(obj_key1, k_oo_idx_1), c_oorder, s_arena.get()/*,OORDER_C_ID_IDX*/);
      }
      ...
      tbl_oorder_c_id_idx(warehouse_id)->rscan(txn, Encode(obj_key0, k_oo_idx_hi), nullptr, c_oorder, s_arena.get()/*,OORDER_C_ID_IDX*/);
    ...
    tbl_order_line(warehouse_id)->scan(txn, Encode(obj_key0, k_ol_0), &Encode(obj_key1, k_ol_1), c_order_line, s_arena.get()/*,ORDER_LINE*/);
...
tpcc_worker::txn_stock_level()
{
...
#endif
#ifdef BCC_TXN_STATS
  bcc_txn_stats_setup(txn, "STOCK_LEVEL");
#endif
  ...
  try {
    ...
    const order_line::key k_ol_1(warehouse_id, districtID, cur_next_o_id, 0);
    {
      ...
      tbl_order_line(warehouse_id)->scan(txn, Encode(obj_key0, k_ol_0), &Encode(obj_key1, k_ol_1), c, s_arena.get()/*,ORDER_LINE*/);
    }
    {
      ...
      for (auto &p : c.s_i_ids) {
        ...
        INVARIANT(p.first >= 1 && p.first <= NumItems());
        {
          ...
          ALWAYS_ASSERT(tbl_stock(warehouse_id)->get(txn, Encode(obj_key0, k_s), obj_v, nbytesread/*, STOCK*/));
        }
...
public:
  tpcc_bench_runner(ndb_wrapper *db, size_t nthreads)
...
bcctest(ndb_wrapper *db, size_t nthreads, int argc, char **argv)
{
  ...
  tpcc_bench_runner r(db, nthreads);
  r.run();
  exit(0); 
}
