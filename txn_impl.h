...
//#define BCC_DBG_PRINT_TUPLE_ADDRESS
#ifdef BCC_DBG_PRINT_TUPLE_ADDRESS
#include <bitset>
#endif
...
transaction<Protocol, Traits>::transaction(uint64_t flags, string_allocator_type &sa)
  : transaction_base(flags), sa(&sa)
    , sh_txn(nullptr), sh_txn_ar(nullptr), tid_arrays(nullptr),
    start_tid(nullptr), end_tid(nullptr),
    tids_valid(nullptr), buf_unique_non_inserts(nullptr), n_unique_non_inserts(0),
    is_end_tid_populated(false),
#ifdef USE_BCC
    hashed_read_set(nullptr),
#endif
    nworkers_(0), tindex(0), launched_threads(0),
    left_conflict(NOCONFLICT), right_conflict(NOCONFLICT)
#ifdef BCC_TXN_STATS
    , t_txn_start(0.0), t_bcc_check_start(0.0), t_bcc_check_end(0.0),
    probed(false), stats(nullptr)
#endif
{
#ifdef BCC_TXN_STATS
  struct timespec t_start;
  clock_gettime(CLOCK_REALTIME, &t_start);
  t_txn_start = TVAL(t_start);
#endif
...
transaction<Protocol, Traits>::~transaction()
{
  ...
  if (stats) {
    ...
    double txn_duration = TVAL(t_end) - t_txn_start;
    if (state == TXN_COMMITED) {
      if (right_conflict == NOCONFLICT)
        stats->add_txn_silo_committed(txn_duration);
      else {
#ifdef USE_BCC
        // These three statistics will only be collected when BCC is enabled.
        stats->add_txn_bcc_committed(txn_duration);
        stats->add_before_bcc_checks_committed(t_bcc_check_start - t_txn_start);
        stats->add_in_bcc_checks_committed(t_bcc_check_end - t_bcc_check_start);
#endif
      }
    } else if (state == TXN_ABRT) {
      if (right_conflict == NOCONFLICT) {
        stats->add_txn_other_aborted(txn_duration);
        if (probed)
          stats->add_probe();
      }
      else {
        stats->add_txn_conflict_aborted(txn_duration);
#ifdef USE_BCC
        // These two statistics will only be collected when BCC is enabled.
        stats->add_before_bcc_checks_aborted(t_bcc_check_start - t_txn_start);
        stats->add_in_bcc_checks_aborted(t_bcc_check_end - t_bcc_check_start);
        if (left_conflict == WRCONFLICT)
          stats->add_txn_wr_left_conflict();
        else if (left_conflict == WWCONFLICT)
          stats->add_txn_ww_left_conflict();
        else if (left_conflict == RWCONFLICT)
          stats->add_txn_rw_left_conflict();
#endif
      }
    }
  }
...
#ifdef USE_BCC
    // If this txn's epoch has not been set.
    if (sh_txn && !sh_txn->is_release_tid_set()) {
      // TODO: If we always insert aborted txn's memory into list_alloc,
      // this may increase the memory pressure of our memory manager in high
      // contention, when the abortion rate may be high. It might be better if
      // we insert aborted txn's memory back to free list.
      sh_txn->set_release(0, true /* aborted */);
      set_sh_tid(0);
    }
#endif
...
#endif
#endif
    // BCC: This aasertion fails at very high contention. Will figure out why.
    //ALWAYS_ASSERT(false);
  }
...
template <template <typename> class Protocol, typename Traits>
bool
transaction<Protocol, Traits>::commit(bool doThrow, uint64_t *tid)
{
#ifdef TUPLE_MAGIC
  try {
#endif
...
#ifdef USE_BCC
          if (buf_unique_non_inserts) {
            ALWAYS_ASSERT(n_unique_non_inserts < BCC_MAX_UNIQUE_NON_INSERTS - 1);
            buf_unique_non_inserts[n_unique_non_inserts++] = last_px->get_tuple();
          }
#endif
          // on boundary
...
#ifdef USE_BCC
      if (buf_unique_non_inserts && likely(last_px)) {
        ALWAYS_ASSERT(n_unique_non_inserts < BCC_MAX_UNIQUE_NON_INSERTS - 1);
        buf_unique_non_inserts[n_unique_non_inserts++] = last_px->get_tuple();
      }
#endif
  ...
  do_bcc_checks:
    {
      // XXX: We may be able to merge such checking with previous
      // read_set/write_set scanning (read_set can be merged; write set
      // checking needs to happen after write set locking). Make sure such
      // merging does not increase low-contention overhead.
      if (right_conflict != NOCONFLICT) {
        ...
        t_bcc_check_start = TVAL(t);
#endif
#ifndef BCC_WR_LEFT_CONFLICT_CHECK_FORWARD
        if (left_conflict == NOCONFLICT && check_wr_left_conflict()) {
          left_conflict = WRCONFLICT;
        }
#endif
        if (left_conflict == NOCONFLICT && check_ww_left_conflict()) {
          left_conflict = WWCONFLICT;
        }
        if (left_conflict == NOCONFLICT && n_unique_non_inserts > 0/*!write_set.empty()*/ &&
            check_rw_left_conflict()) {
          left_conflict = RWCONFLICT;
        }
#ifdef BCC_TXN_STATS
        clock_gettime(CLOCK_REALTIME, &t);
        t_bcc_check_end = TVAL(t);
#endif
        if (left_conflict != NOCONFLICT)
          goto do_abort;
      }

      uint64_t final_tid;
      if (commit_tid.first)
        final_tid = commit_tid.second;
      else  
        final_tid = cast()->gen_abort_tid();
      if (tid)
        *tid = final_tid;
      if (sh_txn)
        sh_txn->set_release(get_release_tid());
      set_sh_tid(final_tid);
    }
#endif  // USE_BCC
    ...
    if (!write_dbtuples.empty()) {
      ...
      typename write_set_map::iterator it     = write_set.begin();
      typename write_set_map::iterator it_end = write_set.end();
      for (uint32_t offset = 0; it != it_end; ++it, ++offset) {
...
#ifdef USE_BCC
  {
    uint64_t final_tid = cast()->gen_abort_tid();
    if (tid)
      *tid = final_tid;
    // If we always insert aborted txn's memory into list_alloc,
    // this may increase the memory pressure of our memory manager in high
    // contention, when the abortion rate may be high. It might be better if
    // we insert aborted txn's memory back to free list.
    if (sh_txn)
...
transaction<Protocol, Traits>::do_tuple_read(
    const dbtuple *tuple, ValueReader &value_reader/*, int table = 0*/)
{
  INVARIANT(tuple);
  ...
  if (!is_snapshot_txn){
#ifdef USE_BCC
    // Must add the tuple to the hash table before actually reading the tuple
    // to avoid missing rw conflicts.
    if (hashed_read_set) {
      hashed_read_set->AddTuple(tuple);
    }
#endif
  }
  ...
  dbtuple::ReadStatus stat;
  {
    ...
    tuple->prefetch();
    stat = tuple->stable_read(is_snapshot_txn, /*sh_ht, *//*table,*/ snapshot_tid, start_t, value_reader, this->string_allocator(), is_snapshot_txn);
  ...
  if (!is_snapshot_txn){
    ...
    if (hashed_read_set /* no bench_loader */ && left_conflict == NOCONFLICT &&
        check_wr_left_conflict2(start_t) && check_rw_right_conflict()) {
      left_conflict = WRCONFLICT;
    }
#endif
#endif
...
{
    /*
     * If using our method, we don't need to keep index node info
     */
//#ifndef USE_BCC  
  INVARIANT(n);
  if (is_snapshot())
